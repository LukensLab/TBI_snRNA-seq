...
data_dir <- file.path(root_dir, 'analysis', '2', 'de', 'wald', 'pairwise-comps', 'size-factors')

## Read and prepare data
read_and_prepare_data <- function(comparison, fc_cutoff, mean_cutoff = 0.1) {
  ...
  for (cell_type in cell_types) {
    ...
    filtered_data <- sub_data[sub_data$qval < 0.01 & abs(sub_data$log2fc) >= fc_cutoff & sub_data$mean >= mean_cutoff, ]
    
    # Store data and filtered gene names
...
comparisons <- c('AC', 'CE', 'AB', 'AD', 'CD', 'DF', 'EF')
fc_cutoffs <- c(0.5, 1)
...
for (comparison in comparisons) {
  for (fc_cutoff in fc_cutoffs) {
    ...
    ## Save plots
    save_dir <- file.path(root_dir, 'analysis', '2', 'figures', 'wald', 'volcano-normalized', paste("logfc", fc_cutoff, comparison, sep="_"))
